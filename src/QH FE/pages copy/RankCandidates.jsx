...
export default function RankCandidates() {
  ...
  const fetchJobAndCandidates = async () => {
    try {
      ...
      const apps = await quantaClient.entities.Application.filter({ job_id: jobId });
      setCandidates(apps || []);
    } catch (error) {
  ...
  const rankAllCandidates = async () => {
    ...
    for (let i = 0; i < appsWithCV.length; i++) {
      ...
      try {
        await quantaClient.functions.invoke("processCV", {
          cv_url: app.cv_url,
  ...
  const handleTopN = async () => {
    ...
    setProcessing(true);
    await Promise.all([
      ...topCandidates.map(c => quantaClient.entities.Application.update(c.id, { status: 'shortlisted' })),
      ...restCandidates.map(c => quantaClient.entities.Application.update(c.id, { status: 'rejected' })),
    ]);
    await fetchJobAndCandidates();
    setProcessing(false);
  ...
  const sendFeedbackToAll = async () => {
    ...
    const res = await quantaClient.functions.invoke("sendCandidateEmails", {
  ...
  const runAgenticRank = async () => {
    ...
    const res = await quantaClient.functions.invoke("agenticRank", {
      job_id: jobId,
      job_title: job?.title || "",
      job_description: job?.description || "",
      job_skills: job?.skills || [],
