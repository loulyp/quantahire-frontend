...
export default function RecruiterApprovalTable() {
  ...
  useEffect(() => {
    const init = async () => {
      try {
        ...
        const data = await quantaClient.asServiceRole.entities.RecruiterProfile.list();
        setRecruiters(data || []);
      } catch (err) {
  ...
  const handleApprove = async () => {
    ...
    await quantaClient.asServiceRole.entities.RecruiterProfile.update(confirmDialog.id, {
      status: "approved",
      is_approved: true
    });
