...
export default function ProposeInterviewModal({ application, recruiterEmail, onClose, onSent }) {
  ...
  const handleSend = async () => {
    ...
    const existing = await quantaClient.entities.InterviewSlot.filter({ application_id: application.id });
    if (existing.length > 0) {
      await quantaClient.entities.InterviewSlot.update(existing[0].id, {
      ...
      await quantaClient.entities.InterviewSlot.create({
    ...
    await quantaClient.integrations.Core.SendEmail({
